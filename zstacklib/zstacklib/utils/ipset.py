...
class IPSet(object):
    ...
    def transform_cmd(self, is_exist=True):
        ...
        ip_cmd_constant = self._create_set_cmd(is_exist) + '\n'.join(self._add_ip_cmd_list(is_exist))
        constant = '%s\n%s\n%s' % (create_cmd_constant, flush_cmd_constant, ip_cmd_constant)
        return constant

    def _create_set_cmd(self, is_exist=True):
        option = ['', '--exist'][is_exist]
        return 'create %s %s family %s %s\n' % (self.name, self.type, self.ip_version, option)

    def _add_ip_cmd_list(self, is_exist=True):
        option = ['', '--exist'][is_exist]
        match_cmd = ['add %s %s' % (ip, option) for ip in self.match_ip]
        nomatch_cmd = ['add %s %s nomatch' % (ip, option) for ip in self.match_ip]
        cmd = match_cmd
...
class IPSetManager(object):
    ...
    def create_set(self, match_ip=None, nomatch_ip=None, name=DEFAULT_NAME, set_type=DEFAULT_TYPE,
                   ip_version=DEFAULT_IP_VERSION):
        self.sets[name] = IPSet(name, set_type, ip_version)
        self.sets[name].set_match_ip(match_ip)
        self.sets[name].set_nomatch_ip(nomatch_ip)
    ...
    def cleanup_other_ipset(self, validate, used_ipset=None):
        ...
        set_list = shell.call("ipset list -n")
        destroy_cmd = ['destroy %s' % x for x in set_list if validate(x) and x not in used_sets]
        tmp = linux.write_to_temp_file('\n'.join(destroy_cmd))
        o = shell.ShellCmd('ipset save -f %s' % tmp)
        o(False)
        if o.return_code != 0:
            logger.warn('fail to cleanup other ipsets, %s' % o.stderr)
        else:
            logger.debug('success cleanup other ipsets')

    def refresh_my_ipsets(self):
        (tmp_fd, tmp_path) = tempfile.mkstemp()
        tmp_fd = os.fdopen(tmp_fd, 'w')
        for ipset in self.sets:
